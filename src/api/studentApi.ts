...
export async function getStudentGroups(studentId: string) {
  const res = await http.get("/api/student/groups", { params: { studentId } });
...
export async function changeStudentGroup(payload: {
  ...
  const res = await http.post("/api/student/change-group", payload);
...
export async function setTrainerPresence(payload: {
  ...
  const res = await http.post("/api/student/trainer-presence", payload);
  return res.data as { ok: true };
}

export async function getStudentInfo(studentId: string) {
  const res = await http.get("/api/student/info", { params: { studentId } });
  return res.data as { info: any };
}

export async function updateStudentInfo(payload: { studentId: string; patch: any }) {
  const res = await http.post("/api/student/info", payload);
  return res.data as { ok: true; info: any };
}

export async function getStudentAttendance(studentId: string) {
  const res = await http.get("/api/student/attendance", { params: { studentId } });
...
export async function setAttendanceMark(payload: {
  ...
  const res = await http.post("/api/student/attendance", payload);
  return res.data as { ok: true };
}

export async function getStudentProgress(studentId: string) {
  const res = await http.get("/api/student/progress", { params: { studentId } });
  return res.data as { progress: any };
}

export async function getStudentNotes(studentId: string) {
  const res = await http.get("/api/student/notes", { params: { studentId } });
  return res.data as { items: any[] };
}
...
export async function createStudentNote(payload: {
  ...
  const res = await http.post("/api/student/notes", payload);
