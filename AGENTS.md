...
| Link Generator | Рекрутинг | Генерация персональных и маркетинговых ссылок | Отдел продаж / Маркетинг |
...
- В URL: `/v1/students/1`
- В mock-данных и сторах: часто используется строка с префиксом `s_1` (например, `currentStudentId`). 
- **КРИТИЧНО**: Перед отправкой на бэкенд префикс `s_` должен быть удалён. 
  ```typescript
  const studentId = parseInt(currentStudentId.replace('s_', ''), 10);
  ```
- При подключении реального API всегда конвертировать в число.
...
**Payments (финансы)**
```
GET    /payments/student/{id}        ← история платежей ученика (старый формат)
GET    /payments/{id}/projects       ← список проектов ученика
GET    /payments/{sid}/projects/{pid}/calendar  ← календарь оплат по проекту
GET    /payments/{sid}/transactions  ← транзакции ученика (параметр project_id в query)
POST   /payments/archive             ← архивация (поддерживает futureAction)
POST   /payments/recalculate-start-date  ← перенос даты старта с пересчетом
...
**Recruitment (Link Generator & CRM)**
```
GET    /recruitment/new-students     ← список новых учеников
GET    /recruitment/link-history     ← история сгенерированных ссылок
POST   /recruitment/generate-link    ← генерация персональной/маркетинговой ссылки
GET    /recruitment/leads            ← лиды из внешней CRM
GET    /recruitment/target-mail      ← рассылки
GET    /recruitment/import-db        ← импорт из внешних источников
```
